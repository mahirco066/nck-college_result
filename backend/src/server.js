...
// بيانات دخول لوحة الإدارة تُقرأ من متغيرات البيئة في Render.
// لا تضع كلمات المرور الحقيقية داخل GitHub.
const ADMIN_USERNAME = process.env.ADMIN_USERNAME || '';
const ADMIN_PASSWORD = process.env.ADMIN_PASSWORD || '';
const ADMIN_SESSION_SECRET = process.env.ADMIN_SESSION_SECRET || '';

const ADMIN_TOKEN_TTL_MS = 8 * 60 * 60 * 1000; // 8 ساعات
...
function verifyAdminToken(token) {
  try {
    ...
    if (!data.username || data.username !== ADMIN_USERNAME) return false;
...
function requireAdmin(req, res) {
  ...
  if (!match || !verifyAdminToken(match[1])) {
    sendJson(res, 401, {
      success: false,
      message: 'جلسة الإدارة غير صالحة أو انتهت. يرجى تسجيل الدخول مرة أخرى.'
    });
    return false;
  }
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (method === 'POST' && pathname === '/api/admin/login') {
      ...
      if (!ADMIN_USERNAME || !ADMIN_PASSWORD || !ADMIN_SESSION_SECRET) {
        ...
        return sendJson(res, 500, {
          success: false,
          message: 'إعدادات تسجيل دخول الإدارة غير مكتملة على الخادم.'
        });
      }
    ...
    // الصفحة الرئيسية / — بوابة النظام واختيار الواجهة
    if (pathname === '/' || pathname === '') {
      const homeFile = path.join(PUBLIC_DIR, 'home.html');
      if (fs.existsSync(homeFile)) {
        res.writeHead(200, { 'Content-Type': 'text/html; charset=utf-8' });
        return fs.createReadStream(homeFile).pipe(res);
      }
    }
